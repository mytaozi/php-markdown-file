...
if (is_file($fullPath) && pathinfo($fullPath, PATHINFO_EXTENSION) === 'md') {
    ...
    if ($items) {
        foreach ($items as $item) {
            ...
            if (is_dir($itemPath)) {
                $files[] = ['type' => 'folder', 'name' => $item, 'path' => $relativePath];
            } elseif (pathinfo($item, PATHINFO_EXTENSION) === 'md') {
    ...
    if (!empty($files)) {
        ...
        foreach ($files as $file) {
            if ($file['type'] === 'folder') {
                ...
                $content .= '<div class="file-item md-item" data-path="' . htmlspecialchars($file['path']) . '"><i class="fas fa-file-alt"></i><span>' . htmlspecialchars($file['name']) . '</span></div>';
            } elseif ($file['type'] === 'file') {
                $content .= '<div class="file-item other-item" data-path="' . htmlspecialchars($file['path']) . '"><i class="fas fa-file"></i><span>' . htmlspecialchars($file['name']) . '</span></div>';
            }
        }
        $content .= '</div>';
    }
                ...
                function renderTree($items) {
                    ...
                    foreach ($items as $item) {
                        ...
                        if ($item['type'] === 'dir') {
                            ...
                            echo '<i class="fas fa-file-alt tree-icon"></i>';
                            echo $displayName;
                            echo '</li>';
                        }
                    }
        ...
        function displaySearchResults(results) {
            ...
            if (results.length === 0) {
                ...
                results.forEach(result => {
                    ...
                    icon.className = result.type === 'file' ? 'fas fa-file-alt' : 'fas fa-folder';
