...
if (is_file($fullPath) && pathinfo($fullPath, PATHINFO_EXTENSION) === 'md') {
    ...
    if (!empty($files)) {
        ...
        foreach ($files as $file) {
            if ($file['type'] === 'folder') {
                $content .= '<div class="file-item folder-item"><i class="fas fa-folder"></i><span>' . htmlspecialchars($file['name']) . '</span></div>';
            } elseif ($file['type'] === 'md') {
                $content .= '<div class="file-item md-item"><i class="fas fa-file-alt"></i><span>' . htmlspecialchars($file['name']) . '</span></div>';
            }
        }
        $content .= '</div>';
    }
        ...
        function addFileItemListeners() {
            ...
            document.querySelectorAll('.file-item.md-item').forEach(item => {
                item.addEventListener('click', () => {
                    ...
                    const folderPath = new URLSearchParams(window.location.search).get('file') || '';
